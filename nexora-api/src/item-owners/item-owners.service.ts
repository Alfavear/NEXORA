import {
  BadRequestException,
  Injectable,
  NotFoundException,
} from '@nestjs/common';
import { PrismaService } from '../prisma/prisma.service';
import { CreateItemOwnerDto } from './dto/create-item-owner.dto';
import { UpdateItemOwnerDto } from './dto/update-item-owner.dto';

@Injectable()
export class ItemOwnersService {
  constructor(private prisma: PrismaService) {}

  private async getCompanyIdByUser(userId: number) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { companyId: true },
    });
    if (!user) throw new BadRequestException('Usuario inválido');
    return user.companyId;
  }

  async create(userId: number, dto: CreateItemOwnerDto) {
    const companyId = await this.getCompanyIdByUser(userId);
    return this.prisma.itemOwner.create({
      data: {
        companyId,
        name: dto.name.trim(),
        isActive: dto.isActive ?? true,
      },
    });
  }

  async findAll(userId: number) {
    const companyId = await this.getCompanyIdByUser(userId);
    return this.prisma.itemOwner.findMany({
      where: { companyId },
      orderBy: { name: 'asc' },
    });
  }

  async findOne(userId: number, id: number) {
    const companyId = await this.getCompanyIdByUser(userId);
    const owner = await this.prisma.itemOwner.findFirst({
      where: { id, companyId },
    });
    if (!owner) throw new NotFoundException('Propietario no encontrado');
    return owner;
  }

  async update(userId: number, id: number, dto: UpdateItemOwnerDto) {
    await this.findOne(userId, id);

    return this.prisma.itemOwner.update({
      where: { id },
      data: {
        name: dto.name?.trim(),
        isActive: dto.isActive,
      },
    });
  }

  async remove(userId: number, id: number) {
    await this.findOne(userId, id);
    return this.prisma.itemOwner.update({
      where: { id },
      data: { isActive: false },
    });
  }
}
