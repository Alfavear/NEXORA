...
export default function ItemBrands() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await itemBrandsApi.list();
      setItems(res.data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) await itemBrandsApi.update(editingItem.id, formData);
      else await itemBrandsApi.create(formData);
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await itemBrandsApi.remove(id); load(); } catch (err: any) { setError(err?.response?.data?.message || 'Error al eliminar marca'); }
  };
    ...
    <>
      <div className="card p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white">Marcas de Artículos</h1>
            <p className="text-sm text-slate-300">Maestro de marcas</p>
          </div>
          <button className="btn-primary" onClick={() => setShowModal(true)}>+ Nueva marca</button>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-32 text-slate-500 animate-pulse text-sm tracking-widest">CARGANDO MARCAS...</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-max text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800">
                {filtered.length === 0 ? (
                  <tr><td colSpan={3} className="px-6 py-12 text-center text-slate-500 italic">No se encontraron marcas registradas</td></tr>
                ) : filtered.map((item) => (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-8 w-full max-w-md shadow-[0_0_50px_-12px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
              <div>
                <h2 className="text-2xl font-bold text-white">{editingItem ? 'Editar Marca' : 'Nueva Marca'}</h2>
                <p className="text-xs text-slate-400 mt-1">Define el nombre de la marca comercial.</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Nombre de la Marca *</label>
                <input 
              ...
              <div className="flex items-center gap-3 p-3 bg-slate-900/50 rounded-xl border border-slate-800">
                ...
                />
                <label htmlFor="active" className="text-xs font-semibold text-slate-300 cursor-pointer">MARCA ACTIVA PARA OPERACIONES</label>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="btn-primary flex-1 py-3 font-bold tracking-widest text-[10px] shadow-lg shadow-indigo-500/25">{editingItem ? 'ACTUALIZAR' : 'CREAR MARCA'}</button>
              </div>
            </form>
          </div>
        </div>
